"""MCP-layer tests for the heddle reverify-worklist consumer (federation Seam 2A).

Exercises ``heddle_worklist_ingest`` via call_tool() — the same in-process MCP
...
class TestIngestHeddleWorklistMCP:
    async def test_preview_default_no_write(self, mcp_db: FiligreeDB) -> None:
        result = _parse(await call_tool("heddle_worklist_ingest", {"worklist": _worklist("loomweave:eid:A1")}))
    ...
    async def test_apply_files_and_binds(self, mcp_db: FiligreeDB) -> None:
        result = _parse(
            await call_tool("heddle_worklist_ingest", {"worklist": _worklist("loomweave:eid:A2", priority="P1"), "apply": True})
        )
    ...
    async def test_links_existing_open_issue(self, mcp_db: FiligreeDB) -> None:
        ...
        result = _parse(await call_tool("heddle_worklist_ingest", {"worklist": _worklist("loomweave:eid:A3"), "apply": True}))
    ...
    async def test_worklist_must_be_object(self, mcp_db: FiligreeDB) -> None:
        result = _parse(await call_tool("heddle_worklist_ingest", {"worklist": "nope"}))
        assert result["code"] == ErrorCode.VALIDATION

    async def test_priority_out_of_range_rejected(self, mcp_db: FiligreeDB) -> None:
        result = _parse(await call_tool("heddle_worklist_ingest", {"worklist": _worklist("loomweave:eid:A4"), "priority": 9}))
        assert result["code"] == ErrorCode.VALIDATION
