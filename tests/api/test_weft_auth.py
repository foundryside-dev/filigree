"""Opt-in bearer-token auth for the weft federation surface.

Design: docs/superpowers/specs/2026-06-03-weft-bearer-token-auth-design.md
Issue: filigree-30cd35bcb9 (option b — weft routes honour the bearer token).
"""
...
class TestIsWeftScopedPath:
    ...
    def test_weft_scoped_paths_are_true(self, path: str) -> None:
        assert is_weft_scoped_path(path) is True
    ...
    def test_non_weft_paths_are_false(self, path: str) -> None:
        assert is_weft_scoped_path(path) is False

    def test_every_living_surface_route_is_weft_scoped(self) -> None:
        ...
        for route in routes:
            ...
            assert is_weft_scoped_path(f"/api{route.path}") is True, route.path
            assert is_weft_scoped_path(f"/api/p/acme{route.path}") is True, route.path

    def test_classic_v1_observations_is_weft_scoped(self) -> None:
        ...
        """
        assert is_weft_scoped_path("/api/v1/observations") is True
        assert is_weft_scoped_path("/api/p/acme/v1/observations") is True

    def test_every_federation_write_alias_is_weft_scoped(self) -> None:
        ...
        for router in ungenerationed:
            for r in router.routes:
                if isinstance(r, APIRoute) and r.path.rsplit("/", 1)[-1] in LIVING_FEDERATION_ALIASES:
                    full = f"/api{r.path}"
                    assert is_weft_scoped_path(full) is True, full
                    # server-mode mount /api/p/{key}/... must gate identically
                    assert is_weft_scoped_path(full.replace("/api/", "/api/p/acme/", 1)) is True, full
                    checked += 1
        for router in weft:
            for r in router.routes:
                if isinstance(r, APIRoute) and r.path.rsplit("/", 1)[-1] in LIVING_FEDERATION_ALIASES:
                    assert is_weft_scoped_path(f"/api/weft{r.path}") is True, r.path
                    checked += 1
...
class TestWeftAuthEnforcement:
    async def test_weft_route_open_when_token_unset(self, app_factory: Callable[[str | None], FastAPI]) -> None:
        """Back-compat: with no token configured, weft routes need no auth."""
        app = app_factory(None)
        async with _client(app) as c:
            resp = await c.get("/api/weft/issues")
        assert resp.status_code == 200

    async def test_weft_route_correct_token(self, app_factory: Callable[[str | None], FastAPI]) -> None:
    ...
    async def test_weft_route_absent_header_rejected(self, app_factory: Callable[[str | None], FastAPI]) -> None:
        app = app_factory(TOKEN)
        async with _client(app) as c:
            resp = await c.get("/api/weft/issues")
    ...
    async def test_weft_route_wrong_token_rejected(self, app_factory: Callable[[str | None], FastAPI]) -> None:
        app = app_factory(TOKEN)
        async with _client(app) as c:
            resp = await c.get("/api/weft/issues", headers={"Authorization": "Bearer wrong"})
        assert resp.status_code == 401

    async def test_weft_route_malformed_header_rejected(self, app_factory: Callable[[str | None], FastAPI]) -> None:
    ...
    async def test_whitespace_token_leaves_surface_open_and_warns(
        ...
        is NOT installed and the weft surface stays open — but an operator who
...
class TestWeftAuthScopeBoundary:
