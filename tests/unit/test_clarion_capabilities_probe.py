...
def test_probe_rejects_unreachable_url_with_unavailable_error() -> None:
    ...
    with pytest.raises(RegistryUnavailableError) as exc:
        probe_loomweave_capabilities("http://127.0.0.1:1", timeout_seconds=0.1)
...
def test_validate_raises_on_api_version_mismatch() -> None:
    ...
    assert exc.value.advertised == EXPECTED_LOOMWEAVE_API_VERSION + 1
    assert exc.value.expected == EXPECTED_LOOMWEAVE_API_VERSION
...
def test_validate_raises_when_loomweave_declines_registry_backend_role() -> None:
...
def test_filigree_db_startup_probe_captures_instance_id_and_api_version(tmp_path: Path) -> None:
    with clarion_stub(instance_id="instance-startup") as (base_url, state):
        ...
        try:
            db.initialize()
            assert db.loomweave_instance_id == "instance-startup"
            assert db.loomweave_api_version == EXPECTED_LOOMWEAVE_API_VERSION
            assert db.loomweave_instance_rotated is False
...
def test_filigree_db_startup_raises_on_api_version_mismatch(tmp_path: Path) -> None:
    with clarion_stub(api_version=EXPECTED_LOOMWEAVE_API_VERSION + 1) as (base_url, _state), pytest.raises(RegistryVersionMismatchError):
        FiligreeDB(
            tmp_path / "filigree.db",
            prefix="test",
            registry_backend="loomweave",
...
def test_filigree_db_startup_probe_failure_with_fallback_downgrades_to_warn(
    ...
    with caplog.at_level(logging.WARNING, logger="filigree.core"):
        ...
        try:
            ...
            assert db.loomweave_api_version is None
...
def test_reprobe_detects_instance_id_rotation_and_flips_banner(
    ...
    """Mid-session re-probe flips ``loomweave_instance_rotated`` when Loomweave was re-indexed."""
...
def test_reprobe_with_matching_instance_id_does_not_set_rotation_flag(tmp_path: Path) -> None:
    with clarion_stub(instance_id="instance-stable") as (base_url, _state):
        ...
        try:
            db.initialize()
            assert db.loomweave_instance_rotated is False
            db.reprobe_loomweave_capabilities()
            assert db.loomweave_instance_rotated is False
            assert db.loomweave_instance_id == "instance-stable"
        finally:
            db.close()
...
def test_reprobe_returns_none_for_local_backend(tmp_path: Path) -> None:
    ...
    try:
        db.initialize()
        assert db.reprobe_loomweave_capabilities() is None
    finally:
        db.close()


def test_skip_capability_probe_leaves_state_unset(tmp_path: Path) -> None:
    """Tests that point at non-Loomweave HTTP servers can opt out via the explicit flag."""
    db = FiligreeDB(
        tmp_path / "filigree.db",
        prefix="test",
        registry_backend="loomweave",
