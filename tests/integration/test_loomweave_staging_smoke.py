...
``FILIGREE_REQUIRE_LIVE_LOOMWEAVE=1``.
"""
...
def test_pinned_staging_loomweave_capabilities_are_compatible() -> None:
    raw_base_url = os.environ.get("LOOMWEAVE_STAGING_BASE_URL", "").strip()
    if not raw_base_url:
        _live_unavailable("LOOMWEAVE_STAGING_BASE_URL is required for the scheduled Live Loomweave Integration lane")

    try:
        base_url = normalize_loomweave_base_url(raw_base_url)
    except ValueError as exc:
        _live_unavailable(f"LOOMWEAVE_STAGING_BASE_URL is invalid: {exc}")
