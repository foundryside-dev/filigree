...
coordinated cross-tool freeze, per Loomweave's migration playbook) — this verb is
...
from filigree.sei_backfill import LoomweaveOutOfSyncError, SeiBackfillError, SeiBackfillReport, run_sei_backfill
...
def sei_backfill_cmd(ctx: click.Context, execute: bool, as_json: bool) -> None:
    ...
    value format changes (locator → ``loomweave:eid:<hex>``).
    ...
    Loomweave can no longer resolve are flagged ORPHAN for review, never dropped.
    """
    dry_run = not execute
    with get_db() as db:
        try:
            ...
            click.echo(json_mod.dumps({"error": str(e), "code": "LOOMWEAVE_OUT_OF_SYNC", "remediation_command": "loomweave analyze"}))
            sys.exit(3)
            ...
            _emit_error(f"Loomweave unavailable: {e}", ErrorCode.REGISTRY_UNAVAILABLE, as_json=as_json)
