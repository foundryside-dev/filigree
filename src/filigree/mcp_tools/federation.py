...
Currently one tool — the warpline reverify-worklist consumer (Seam 2A of
``pm/2026-06-13-warpline-interface-lock.md``). warpline produces a
``warpline.reverify_worklist.v1`` worklist; Filigree is the write-capable
consumer that files-or-links its items as work, explicit-action only.

See :mod:`filigree.warpline_consumer` for the file/link logic and the acceptance
...
def register() -> tuple[list[Tool], dict[str, Callable[..., Any]]]:
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "actor": {
                        "type": "string",
                        "description": "Identity recorded as issue creator / association attached_by (default 'warpline').",
                    },
                    "priority": {
                        "type": "integer",
                        "minimum": 0,
    ...
    handlers: dict[str, Callable[..., Any]] = {
        "ingest_warpline_worklist": _handle_ingest_warpline_worklist,
    }

    return tools, handlers


async def _handle_ingest_warpline_worklist(arguments: dict[str, Any]) -> list[TextContent]:
    from filigree.core import WrongProjectError

    args = _parse_args(arguments, WarplineWorklistIngestArgs)
    ...
    actor, actor_err = _validate_actor(args.get("actor", "warpline"))
    if actor_err:
        return actor_err
    ...
    try:
        ...
        _logger.error("ingest_warpline_worklist storage failure: %s", exc)
