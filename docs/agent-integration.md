...
- **Errors** return `{error: str, code: ErrorCode, details?: dict}` where `code` is one of: `VALIDATION`, `NOT_FOUND`, `CONFLICT`, `INVALID_TRANSITION`, `PERMISSION`, `NOT_INITIALIZED`, `IO`, `INVALID_API_URL`, `FILE_REGISTRY_DISPLACED`, `REGISTRY_UNAVAILABLE`, `LOOMWEAVE_REGISTRY_VERSION_MISMATCH`, `LOOMWEAVE_OUT_OF_SYNC`, `BRIEFING_BLOCKED`, `STOP_FAILED`, `SCHEMA_MISMATCH`, `INTERNAL`. (`LOOMWEAVE_OUT_OF_SYNC` means the installed loomweave registry is present but out of sync; remediation is `loomweave analyze`.) `REGISTRY_UNAVAILABLE` means Loomweave could not be reached or negotiated with in fail-closed loomweave mode (`registry_backend=loomweave`, `loomweave.allow_local_fallback=false`); its `details` carry `cause_kind`, the probed `url`, `backend` (`"loomweave"`) and `hint` — a remedy line specific to `cause_kind`. Only reachability failures (`network`, `timeout`, `http_error`) are fixed by starting Loomweave (`loomweave serve`); `auth` / `auth_token_missing` name the bearer-token env var to export (`loomweave.token_env`, default `WEFT_TOKEN`); `role_declined` / `auth_mode_unsupported` point at Loomweave's serving configuration (or `registry_backend='local'`); `invalid_response` means upgrade Filigree or Loomweave to a matching pair — starting Loomweave will not fix it. Every hint except `invalid_response` also offers `loomweave.allow_local_fallback=true` as an interim way to keep working with local file ids. Surface the `hint` to the user rather than retrying blindly.
...
## Registry-Unavailable (Fail-Closed Loomweave Mode)

When the project runs `registry_backend=loomweave` with `loomweave.allow_local_fallback=false` and Loomweave is down, every surface that opens the project database renders the `REGISTRY_UNAVAILABLE` envelope instead of a traceback: CLI verbs exit 1 (plain mode prints the one-line error plus the remedy on stderr; `--json` prints the envelope with `details.backend` and `details.hint` on stdout), the stdio MCP server starts in degraded mode (`mcp_status_get.status == "registry_unavailable"`, with a `registry_retry` schedule — it re-probes Loomweave at most once per 10 s and recovers without a restart), and the dashboard answers a 503 envelope. The `filigree session-context` hook (which always exits 0) now prints the same error and remedy line inside the project snapshot instead of the generic `Warning: session-context hook failed`, so the remedy reaches the agent at session start. Act on `details.hint` / the printed remedy; do not retry in a loop.
